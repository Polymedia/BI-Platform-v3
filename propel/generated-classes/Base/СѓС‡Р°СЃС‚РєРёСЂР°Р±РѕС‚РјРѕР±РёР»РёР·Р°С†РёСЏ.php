...
/**
 * Base class that represents a row from the 'Участки_работ_мобилизация' table.
 *
 * 
 *
* @package    propel.generator..Base
*/
abstract class участкиработмобилизация implements ActiveRecordInterface 
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\участкиработмобилизацияTableMap';
    ...
    /**
     * Initializes internal state of Base\участкиработмобилизация object.
     */
    ...
    /**
     * Compares this with another <code>участкиработмобилизация</code> instance.  If
     * <code>obj</code> is an instance of <code>участкиработмобилизация</code>, delegates to
     * <code>equals(участкиработмобилизация)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|участкиработмобилизация The current object, for fluid interface
     */
     ...
     * @return $this|\участкиработмобилизация The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[участкиработмобилизацияTableMap::COL_ID] = true;
        }
    ...
    public function setучастокработ($v)
    {
        ...
        if ($this->участок_работ !== $v) {
            ...
            $this->modifiedColumns[участкиработмобилизацияTableMap::COL_УЧАСТОК_РАБОТ] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : участкиработмобилизацияTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : участкиработмобилизацияTableMap::translateFieldName('участокработ', TableMap::TYPE_PHPNAME, $indexType)];
            $this->участок_работ = (null !== $col) ? (string) $col : null;
            $this->resetModified();
            ...
            return $startcol + 2; // 2 = участкиработмобилизацияTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\участкиработмобилизация'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildучасткиработмобилизацияQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see участкиработмобилизация::setDeleted()
     * @see участкиработмобилизация::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildучасткиработмобилизацияQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                участкиработмобилизацияTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(участкиработмобилизацияTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(участкиработмобилизацияTableMap::COL_УЧАСТОК_РАБОТ)) {
            $modifiedColumns[':p' . $index++]  = 'Участок_работ';
        }

        $sql = sprintf(
            'INSERT INTO Участки_работ_мобилизация (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'id':                        
                        ...
                        break;
                    case 'Участок_работ':                        
                        $stmt->bindValue($identifier, $this->участок_работ, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    {
        $pos = участкиработмобилизацияTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['участкиработмобилизация'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['участкиработмобилизация'][$this->hashCode()] = true;
        $keys = участкиработмобилизацияTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getучастокработ(),
        );
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\участкиработмобилизация
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setучастокработ($arr[$keys[1]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(участкиработмобилизацияTableMap::COL_ID)) {
            $criteria->add(участкиработмобилизацияTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(участкиработмобилизацияTableMap::COL_УЧАСТОК_РАБОТ)) {
            $criteria->add(участкиработмобилизацияTableMap::COL_УЧАСТОК_РАБОТ, $this->участок_работ);
        }
    ...
    {
        $criteria = ChildучасткиработмобилизацияQuery::create();
        $criteria->add(участкиработмобилизацияTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \участкиработмобилизация (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \участкиработмобилизация Clone of current object.
     * @throws PropelException
     */
     ...
     * If this Childучасткиработмобилизация is new, it will return
        ...
        if (null === $this->collмобилизацияs || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collмобилизацияs) {
                ...
                $collмобилизацияs = ChildмобилизацияQuery::create(null, $criteria)
                    ->filterByучасткиработмобилизация($this)
                    ->find($con);
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|Childучасткиработмобилизация The current object (for fluent API support)
     */
    public function setмобилизацияs(Collection $�обилизацияs, ConnectionInterface $con = null)
    {
        ...
        foreach ($�обилизацияsToDelete as $�обилизацияRemoved) {
            $�обилизацияRemoved->setучасткиработмобилизация(null);
        }
    ...
    public function removeмобилизация(Childмобилизация $�обилизация)
    {
        if ($this->getмобилизацияs()->contains($�обилизация)) {
            ...
            $�обилизация->setучасткиработмобилизация(null);
        }
     ...
     * Otherwise if this участкиработмобилизация is new, it will return
     * an empty collection; or if this участкиработмобилизация has previously
     * been saved, it will retrieve related мобилизацияs from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in участкиработмобилизация.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
    ...
    public function setмобилизацияпомесяцамs(Collection $�обилизацияпомесяцамs, ConnectionInterface $con = null)
    {
        ...
        foreach ($�обилизацияпомесяцамsToDelete as $�обилизацияпомесяцамRemoved) {
            $�обилизацияпомесяцамRemoved->setучасткиработмобилизация(null);
        }
    ...
    protected function doAddмобилизацияпомесяцам(Childмобилизацияпомесяцам $�обилизацияпомесяцам)
    {
        ...
        $�обилизацияпомесяцам->setучасткиработмобилизация($this);
    }
    ...
    public function removeмобилизацияпомесяцам(Childмобилизацияпомесяцам $�обилизацияпомесяцам)
    {
        if ($this->getмобилизацияпомесяцамs()->contains($�обилизацияпомесяцам)) {
            ...
            $�обилизацияпомесяцам->setучасткиработмобилизация(null);
        }
    ...
    {
        return (string) $this->exportTo(участкиработмобилизацияTableMap::DEFAULT_STRING_FORMAT);
    }
