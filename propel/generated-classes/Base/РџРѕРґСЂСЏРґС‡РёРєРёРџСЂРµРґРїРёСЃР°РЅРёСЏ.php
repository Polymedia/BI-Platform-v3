...
/**
 * Base class that represents a row from the 'Подрядчики_предписания' table.
 *
 * 
 *
* @package    propel.generator..Base
*/
abstract class ПодрядчикиПредписания implements ActiveRecordInterface 
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\ПодрядчикиПредписанияTableMap';
    ...
    /**
     * Initializes internal state of Base\ПодрядчикиПредписания object.
     */
    ...
    /**
     * Compares this with another <code>ПодрядчикиПредписания</code> instance.  If
     * <code>obj</code> is an instance of <code>ПодрядчикиПредписания</code>, delegates to
     * <code>equals(ПодрядчикиПредписания)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|ПодрядчикиПредписания The current object, for fluid interface
     */
     ...
     * @return $this|\ПодрядчикиПредписания The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[ПодрядчикиПредписанияTableMap::COL_ID] = true;
        }
    ...
    public function setподрядчик($v)
    {
        ...
        if ($this->подрядчик !== $v) {
            $this->подрядчик = $v;
            $this->modifiedColumns[ПодрядчикиПредписанияTableMap::COL_ПОДРЯДЧИК] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ПодрядчикиПредписанияTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ПодрядчикиПредписанияTableMap::translateFieldName('подрядчик', TableMap::TYPE_PHPNAME, $indexType)];
            $this->подрядчик = (null !== $col) ? (string) $col : null;
            $this->resetModified();
            ...
            return $startcol + 2; // 2 = ПодрядчикиПредписанияTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\ПодрядчикиПредписания'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildПодрядчикиПредписанияQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see ПодрядчикиПредписания::setDeleted()
     * @see ПодрядчикиПредписания::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildПодрядчикиПредписанияQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                ПодрядчикиПредписанияTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ПодрядчикиПредписанияTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(ПодрядчикиПредписанияTableMap::COL_ПОДРЯДЧИК)) {
            $modifiedColumns[':p' . $index++]  = 'Подрядчик';
        }

        $sql = sprintf(
            'INSERT INTO Подрядчики_предписания (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'id':                        
                        ...
                        break;
                    case 'Подрядчик':                        
                        $stmt->bindValue($identifier, $this->подрядчик, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    {
        $pos = ПодрядчикиПредписанияTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['ПодрядчикиПредписания'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['ПодрядчикиПредписания'][$this->hashCode()] = true;
        $keys = ПодрядчикиПредписанияTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getподрядчик(),
        );
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\ПодрядчикиПредписания
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ПодрядчикиПредписанияTableMap::COL_ID)) {
            $criteria->add(ПодрядчикиПредписанияTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(ПодрядчикиПредписанияTableMap::COL_ПОДРЯДЧИК)) {
            $criteria->add(ПодрядчикиПредписанияTableMap::COL_ПОДРЯДЧИК, $this->подрядчик);
        }
    ...
    {
        $criteria = ChildПодрядчикиПредписанияQuery::create();
        $criteria->add(ПодрядчикиПредписанияTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \ПодрядчикиПредписания (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \ПодрядчикиПредписания Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildПодрядчикиПредписания is new, it will return
        ...
        if (null === $this->collПредписанияs || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collПредписанияs) {
                ...
                $collПредписанияs = ChildПредписанияQuery::create(null, $criteria)
                    ->filterByПодрядчикиПредписания($this)
                    ->find($con);
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildПодрядчикиПредписания The current object (for fluent API support)
     */
    public function setПредписанияs(Collection $�редписанияs, ConnectionInterface $con = null)
    {
        ...
        foreach ($�редписанияsToDelete as $�редписанияRemoved) {
            $�редписанияRemoved->setПодрядчикиПредписания(null);
        }
    ...
    protected function doAddПредписания(ChildПредписания $�редписания)
    {
        ...
        $�редписания->setПодрядчикиПредписания($this);
    }
    ...
    public function removeПредписания(ChildПредписания $�редписания)
    {
        if ($this->getПредписанияs()->contains($�редписания)) {
            ...
            $�редписания->setПодрядчикиПредписания(null);
        }
     ...
     * Otherwise if this ПодрядчикиПредписания is new, it will return
     * an empty collection; or if this ПодрядчикиПредписания has previously
     * been saved, it will retrieve related Предписанияs from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in ПодрядчикиПредписания.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
    ...
    {
        return (string) $this->exportTo(ПодрядчикиПредписанияTableMap::DEFAULT_STRING_FORMAT);
    }
