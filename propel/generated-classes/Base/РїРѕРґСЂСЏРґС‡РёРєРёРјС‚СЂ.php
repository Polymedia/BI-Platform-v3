...
/**
 * Base class that represents a row from the 'Подрядчики_МТР' table.
 *
 * 
 *
* @package    propel.generator..Base
*/
abstract class подрядчикимтр implements ActiveRecordInterface 
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\подрядчикимтрTableMap';
    ...
    /**
     * Initializes internal state of Base\подрядчикимтр object.
     */
    ...
    /**
     * Compares this with another <code>подрядчикимтр</code> instance.  If
     * <code>obj</code> is an instance of <code>подрядчикимтр</code>, delegates to
     * <code>equals(подрядчикимтр)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|подрядчикимтр The current object, for fluid interface
     */
     ...
     * @return $this|\подрядчикимтр The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[подрядчикимтрTableMap::COL_ID] = true;
        }
    ...
    public function setподрядчик($v)
    {
        ...
        if ($this->подрядчик !== $v) {
            $this->подрядчик = $v;
            $this->modifiedColumns[подрядчикимтрTableMap::COL_ПОДРЯДЧИК] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : подрядчикимтрTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : подрядчикимтрTableMap::translateFieldName('подрядчик', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = подрядчикимтрTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\подрядчикимтр'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildподрядчикимтрQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see подрядчикимтр::setDeleted()
     * @see подрядчикимтр::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildподрядчикимтрQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                подрядчикимтрTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[подрядчикимтрTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . подрядчикимтрTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(подрядчикимтрTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(подрядчикимтрTableMap::COL_ПОДРЯДЧИК)) {
            $modifiedColumns[':p' . $index++]  = 'Подрядчик';
        }

        $sql = sprintf(
            'INSERT INTO Подрядчики_МТР (%s) VALUES (%s)',
    ...
    {
        $pos = подрядчикимтрTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['подрядчикимтр'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['подрядчикимтр'][$this->hashCode()] = true;
        $keys = подрядчикимтрTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
    ...
    {
        $criteria = new Criteria(подрядчикимтрTableMap::DATABASE_NAME);

        if ($this->isColumnModified(подрядчикимтрTableMap::COL_ID)) {
            $criteria->add(подрядчикимтрTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(подрядчикимтрTableMap::COL_ПОДРЯДЧИК)) {
            $criteria->add(подрядчикимтрTableMap::COL_ПОДРЯДЧИК, $this->подрядчик);
        }
    ...
    {
        $criteria = ChildподрядчикимтрQuery::create();
        $criteria->add(подрядчикимтрTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \подрядчикимтр (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \подрядчикимтр Clone of current object.
     * @throws PropelException
     */
     ...
     * If this Childподрядчикимтр is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|Childподрядчикимтр The current object (for fluent API support)
     */
    public function setмтрs(Collection $�трs, ConnectionInterface $con = null)
    {
        ...
        foreach ($�трsToDelete as $�трRemoved) {
            $�трRemoved->setподрядчикимтр(null);
        }
     ...
     * Otherwise if this подрядчикимтр is new, it will return
     * an empty collection; or if this подрядчикимтр has previously
     * been saved, it will retrieve related мтрs from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in подрядчикимтр.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
    ...
    {
        return (string) $this->exportTo(подрядчикимтрTableMap::DEFAULT_STRING_FORMAT);
    }
