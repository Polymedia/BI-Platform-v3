...
/**
 * Base class that represents a row from the 'ПроблемныеВопросы' table.
 *
 * 
 *
...
abstract class Проблемныевопросы implements ActiveRecordInterface 
{
    ...
    /**
     * The value for the проблемныевопросы field.
     * 
     * @var        string
     */
    protected $проблемныевопросы;

    /**
     * The value for the мерыдлярешения field.
     * 
     * @var        string
     */
    protected $мерыдлярешения;
    ...
    /**
     * Get the [проблемныевопросы] column value.
     * 
     * @return string
     */
    ...
    /**
     * Get the [мерыдлярешения] column value.
     * 
     * @return string
     */
    public function getМерыдлярешения()
    {
        return $this->мерыдлярешения;
    }
    ...
    /**
     * Set the value of [проблемныевопросы] column.
     * 
     * @param string $v new value
     * @return $this|\Проблемныевопросы The current object (for fluent API support)
     */
    public function setПроблемныевопросы($v)
    {
        ...
        if ($this->проблемныевопросы !== $v) {
            $this->проблемныевопросы = $v;
            $this->modifiedColumns[ПроблемныевопросыTableMap::COL_ПРОБЛЕМНЫЕВОПРОСЫ] = true;
        }
    ...
    /**
     * Set the value of [мерыдлярешения] column.
     * 
     * @param string $v new value
     * @return $this|\Проблемныевопросы The current object (for fluent API support)
     */
    public function setМерыдлярешения($v)
    {
        ...
        if ($this->мерыдлярешения !== $v) {
            $this->мерыдлярешения = $v;
            $this->modifiedColumns[ПроблемныевопросыTableMap::COL_МЕРЫДЛЯРЕШЕНИЯ] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $this->проблемныевопросы = (null !== $col) ? (string) $col : null;
            ...
            $this->мерыдлярешения = (null !== $col) ? (string) $col : null;
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ПроблемныевопросыTableMap::COL_ПРОБЛЕМНЫЕВОПРОСЫ)) {
            $modifiedColumns[':p' . $index++]  = 'ПроблемныеВопросы';
        }
        if ($this->isColumnModified(ПроблемныевопросыTableMap::COL_МЕРЫДЛЯРЕШЕНИЯ)) {
            $modifiedColumns[':p' . $index++]  = 'МерыДляРешения';
        }
        ...
        $sql = sprintf(
            'INSERT INTO ПроблемныеВопросы (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'Кому':                        
                        ...
                        break;
                    case 'ПроблемныеВопросы':                        
                        $stmt->bindValue($identifier, $this->проблемныевопросы, PDO::PARAM_STR);
                        break;
                    case 'МерыДляРешения':                        
                        $stmt->bindValue($identifier, $this->мерыдлярешения, PDO::PARAM_STR);
                        break;
                    case 'Ответсвенный':                        
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ПроблемныевопросыTableMap::COL_ПРОБЛЕМНЫЕВОПРОСЫ)) {
            $criteria->add(ПроблемныевопросыTableMap::COL_ПРОБЛЕМНЫЕВОПРОСЫ, $this->проблемныевопросы);
        }
        if ($this->isColumnModified(ПроблемныевопросыTableMap::COL_МЕРЫДЛЯРЕШЕНИЯ)) {
            $criteria->add(ПроблемныевопросыTableMap::COL_МЕРЫДЛЯРЕШЕНИЯ, $this->мерыдлярешения);
        }
        if ($this->isColumnModified(ПроблемныевопросыTableMap::COL_ОТВЕТСВЕННЫЙ)) {
    ...
    public function clear()
    {
        ...
        $this->дата = null;
        $this->кому = null;
        $this->проблемныевопросы = null;
        $this->мерыдлярешения = null;
