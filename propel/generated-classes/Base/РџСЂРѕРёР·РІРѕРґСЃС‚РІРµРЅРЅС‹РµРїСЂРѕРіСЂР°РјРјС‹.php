...
/**
 * Base class that represents a row from the 'ПроизводственныеПрограммы' table.
 *
 * 
 *
...
abstract class Производственныепрограммы implements ActiveRecordInterface 
{
    ...
    /**
     * The value for the типпрограммы field.
     * 
     * @var        int
     */
    protected $типпрограммы;
    ...
    /**
     * Get the [типпрограммы] column value.
     * 
     * @return int
     */
    public function getТиппрограммы()
    {
        return $this->типпрограммы;
    }
    ...
    /**
     * Set the value of [типпрограммы] column.
     * 
     * @param int $v new value
     * @return $this|\Производственныепрограммы The current object (for fluent API support)
     */
    public function setТиппрограммы($v)
    {
        ...
        if ($this->типпрограммы !== $v) {
            $this->типпрограммы = $v;
            $this->modifiedColumns[ПроизводственныепрограммыTableMap::COL_ТИППРОГРАММЫ] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $this->типпрограммы = (null !== $col) ? (int) $col : null;
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ПроизводственныепрограммыTableMap::COL_ТИППРОГРАММЫ)) {
            $modifiedColumns[':p' . $index++]  = 'ТипПрограммы';
        }
        ...
        $sql = sprintf(
            'INSERT INTO ПроизводственныеПрограммы (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'id':                        
                        ...
                        break;
                    case 'ТипПрограммы':                        
                        $stmt->bindValue($identifier, $this->типпрограммы, PDO::PARAM_INT);
                        break;
                    case 'Год':                        
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ПроизводственныепрограммыTableMap::COL_ТИППРОГРАММЫ)) {
            $criteria->add(ПроизводственныепрограммыTableMap::COL_ТИППРОГРАММЫ, $this->типпрограммы);
        }
        if ($this->isColumnModified(ПроизводственныепрограммыTableMap::COL_ГОД)) {
