...
/**
 * Base class that represents a row from the 'ТипыТехникиМТР' table.
 *
 * 
 *
...
abstract class Типытехникимтр implements ActiveRecordInterface 
{
    ...
    public function setТиптехники($v)
    {
        ...
        if ($this->типтехники !== $v) {
            $this->типтехники = $v;
            $this->modifiedColumns[ТипытехникимтрTableMap::COL_ТИПТЕХНИКИ] = true;
        }
    ...
    public function setОтображаемыйтиптехники($v)
    {
        ...
        if ($this->отображаемыйтиптехники !== $v) {
            ...
            $this->modifiedColumns[ТипытехникимтрTableMap::COL_ОТОБРАЖАЕМЫЙТИПТЕХНИКИ] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $this->отображаемыйтиптехники = (null !== $col) ? (int) $col : null;
            $this->resetModified();
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ТипытехникимтрTableMap::COL_ТИПТЕХНИКИ)) {
            $modifiedColumns[':p' . $index++]  = 'ТипТехники';
        }
        if ($this->isColumnModified(ТипытехникимтрTableMap::COL_ОТОБРАЖАЕМЫЙТИПТЕХНИКИ)) {
            $modifiedColumns[':p' . $index++]  = 'ОтображаемыйТипТехники';
        }

        $sql = sprintf(
            'INSERT INTO ТипыТехникиМТР (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'ОтображаемыйТипТехники':                        
                        $stmt->bindValue($identifier, $this->отображаемыйтиптехники, PDO::PARAM_INT);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ТипытехникимтрTableMap::COL_ТИПТЕХНИКИ)) {
            $criteria->add(ТипытехникимтрTableMap::COL_ТИПТЕХНИКИ, $this->типтехники);
        }
        if ($this->isColumnModified(ТипытехникимтрTableMap::COL_ОТОБРАЖАЕМЫЙТИПТЕХНИКИ)) {
            $criteria->add(ТипытехникимтрTableMap::COL_ОТОБРАЖАЕМЫЙТИПТЕХНИКИ, $this->отображаемыйтиптехники);
        }
